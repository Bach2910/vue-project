  ...
  <div>
    <div class="people-header-title container">
      ...
      <div class="description">
        <p>Testcenter đưa đến nguồn tài nguyên dồi dào liên quan đến mô hình quản trị doanh nghiệp, công cụ tạo bài test
          online, tài liệu hướng dẫn và cách thức đưa các bài kiểm tra vào quy trình vận hành doanh nghiệp hiệu
          quả. </p>
      </div>
    </div>
    <div v-for="(article, index) in articles" :key="index" class="article-card">
      <div class="td-module-thumb mb-2">
        <a :href="article.url" rel="bookmark" class="td-image-wrap" :title="article.title">
          <img class="entry-thumb animation" :src="article.image_url" :alt="article.title" width="324"
               height="235"/>
        </a>
      </div>
      <div class="item-details">
        ...
        <div class="module-meta-info mb-2">
          <a href="/blog/tai-lieu/" class="td-post-category">Tài liệu</a>
        </div>
...
const articles = ref([
  {
    ...
    excerpt: 'Mô tả ngắn về bài viết 1'
  },
  {
    title: 'Bài viết 2',
    url: '/url-2',
    image_url: '/image2.jpg',
    excerpt: 'Mô tả ngắn về bài viết 2'
  },
...
const performSearch = () => {
  if (searchQuery.value) {
    ...
    // Ví dụ: Bạn có thể lọc bài viết theo từ khóa tìm kiếm
    const filteredArticles = articles.value.filter(article =>
      article.title.toLowerCase().includes(searchQuery.value.toLowerCase())
    );
    console.log(filteredArticles);  // Hiển thị bài viết đã lọc
  }
};
</script>
